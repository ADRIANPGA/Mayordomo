...
public class Commands {
	...
	public void commandCREATE() {
		...
		if (args.length == 1) {
			channelName.append(ChatUtils.LOCK).append(" Private (").append(capacity).append(" Máx)");
		} else {
		...
		textChannel.sendMessage(ChatUtils.APPROVE + " Canal " + channelName.toString() + " creado con éxito.")
				.complete();
		...
		if (member.getVoiceState().getChannel() != null) {
			try {
				...
				ChatUtils.sendErrorMessage(textChannel, "No se te ha podido mover al canal " + member.getAsMention()
						+ ". Quizás mayordomo no puede ver el canal en el que estás");
	...
	public void commandHELP() {
		...
		for (String string : CommandManager.getAllAliases("channelcapacity"))
			str.append(string).append(", ");
		str.delete(str.length() - 2, str.length());
		embed.addField(str.toString(), "Cambiar los miembros máximos de tu canal\nEj: " + this.aliaseUsed
